...
* 想把一个高频流程沉淀下来，最后写出的 prompt或者是skill 只能**应付当下任务**，下一次仍然要重新写 prompt。
* vibe coding根本看不懂Agent生成的代码或者是方案。遇到不熟悉的技术点时，往往需要先**补齐背景知识**，把核心原理、关键术语和项目落点讲清楚，否则**后续项目开发的方向根本无法把握**。
* 任务依赖最新官方文档、论文、API 或 benchmark，Agent 却凭旧知识给出**看似确定实则风险很高**的结论，后续开发无法摆脱**模型幻觉**。

<p align="center">
  <img src="readme_assets/manage_AI_team.jpg" alt="学会Vibe Coding工作流" width="300">
</p>

`vibe-coding-template` 处理的就是这些协作问题。它不绑定具体业务，也不替项目规定唯一开发方式。你可以把它当成一套可调整的协作模板，按目标项目的真实内容落地。

这套模板不是凭空设计出来的，而是我在多个项目里反复使用 Agent、反复踩坑之后总结出来的。这里面既包括当前的浏览器拓展项目 [CodeNote Helper 的项目级 skills](https://github.com/Philip-Cao-9527/code-note-helper/tree/main/.agents/skills)，也包括我目前在开发的面向复杂表格任务的Agent 工作流和 benchmark 评测的 [TableCodeAgent 项目级 skills](https://github.com/Philip-Cao-9527/TableCodeAgent/tree/main/.agents/skills)。它的重点不是把某个项目的规则原样复制出去，而是把这些项目里反复出现的问题提炼成可以迁移的**协作模板**。
...
`vibe-coding-template` 的核心把 Agent 协作拆解为几类稳定问题。**项目规则怎么沉淀，复杂任务怎么先决策，踩过的坑怎么变成下次不会再犯的约束。**

### agents-md-creator

[agents-md-creator](skills/agents-md-creator/SKILL.md) 负责把项目长期规则写进 `AGENTS.md`。很多 Agent 协作问题，表面上是某次 prompt 没写好，实际原因是项目规则没有沉淀下来。每次都靠临时提醒，Agent 很容易忘记内容质量要求、测试方法、版本策略、修改报告记录格式、编码要求和错误处理边界。
...
它的作用是让 Agent 的工作过程**可观察、可追溯、可复盘**。用户不用每次都追问现在做到哪一步，也能看到当前进度在哪里，哪些事情还没有验证。


<p align="center">
  <img src="readme_assets/progress_report.png" alt="进度播报" width="500">
</p>
...
当时在大语言模型 API 流式输出链路里，API 客户端被Codex**偷偷**加了一个固定 90 秒超时。
...
所以 `AGENTS.md` 里需要明确写清楚，不要无依据加入固定超时、长度截断、条数上限、重试上限等防御性编程的逻辑。如果确实要加限制，必须说明依据、触发条件、可见行为、误伤风险和验证方式。


#### 最小化修改原则的边界

`AGENTS.md` 里需要写清楚默认做**最小必要改动**，但也要说明**最小化修改不是盲目保守**。如果只写最小化修改，Agent 很容易把任务理解成只改小范围的几行代码，最后做出不符合真实用户需求、工业场景或业务实践的方案。更常见的问题是，两个平行功能 A 和 B 本来应该保持一致，Agent 只在 A 里补了新设计，B 里却继续保留旧链路，后续维护会变得越来越割裂。

所以模板需要明确边界。面向真实用户需求、解决反复多次仍未修好的 bug，或者为了贴合真实工业场景和业务实践时，可以允许进行更大范围重构。但大改前必须先说明**根因、必要性、影响范围、回归方案和可回滚边界**。这样既不会让 Agent 随意扩大改动，也不会让它为了表面上的最小而牺牲项目长期一致性。

#### 为什么要记录每一次 Agent 修改

很多项目里，Agent 改完代码后只留下一句已修复或者已完成，时间一长就很难回溯真实变化。尤其是连续多轮修改时，如果没有文档记录，很容易忘记某次改动为什么发生、验证过哪些路径、哪些风险其实还没有闭环。

[agents-md-creator](skills/agents-md-creator/SKILL.md) 的模板会把**修改记录要求**放进 `AGENTS.md`。它要求 Agent 在完成改动后，把改动范围、验证方式、证据路径、未验证边界和风险说明写清楚。文档记录不是为了增加流程负担，而是为了让后续维护者能复盘每一次 Agent 修改，而不是只能从 diff 里猜测当时的判断。
...
我在当前项目 CodeNote Helper 的 [Google Drive 授权问题](https://github.com/Philip-Cao-9527/code-note-helper/commit/23eff5a8a75df038817b6841bbd82645f618202b)上就遇到过这种情况。**多轮 code review 和代码修改都没有定位到关键原因**，后来结合互联网最新信息才确认，Microsoft Edge 浏览器不能沿用 Chrome 浏览器的设计路径使用 `chrome.identity.getAuthToken`，**Edge 浏览器需要采用 `launchWebAuthFlow`**。这意味着 refresh token、长期授权、后台自动同步和 OAuth Client 类型都会受到**浏览器差异**影响。
...
<p align="center">
  <img src="readme_assets/vibe_coding_puzzle.jpg" alt="knowledge-explainer 设计哲学" width="300">
</p>
...
| [validate_knowledge_explainer.py](skills/knowledge-explainer/scripts/validate_knowledge_explainer.py) | 检查讲解类 `SKILL.md` 是否保留 frontmatter、最高约束、1000 / 3000 字门槛、输出失败规则、讲解结构、质量门槛、自检项、未实现能力边界和 [web-search](skills/web-search/SKILL.md) 联动。                                           | `python -X utf8 vibe-coding-template/skills/knowledge-explainer/scripts/validate_knowledge_explainer.py path/to/knowledge-explainer/SKILL.md` |
...
我建议不要一上来就追求把整套模板直接照搬。首先按照下列模板更新一下根目录的`AGENTS.md`。
...
然后可以先用 [agents-md-creator](https://github.com/Philip-Cao-9527/code-note-helper/blob/main/vibe-coding-template/skills/agents-md-creator/SKILL.md)生成一份具体项目的 AGENTS.md。这份文件不用写很长，核心就是让 Agent 进入项目后知道这个项目是什么、开始修改前必须看什么、哪些边界不能碰、怎么证明任务真的完成了。个人建议优先写清楚项目的主目标、技术栈和关键目录，避免 Agent 按自己的想象理解仓库。代码输出的质量要求，修改前必须阅读的 README、开发日志、测试说明和相关文件，当前项目真实可用的测试命令，版本号和修改报告记录这些长期维护规则，还有禁止无依据的防御性编程。最终交付时必须说明改了什么、怎么验证、证据在哪里、还有哪些风险没有覆盖。

写好这些之后不要马上拿大任务来试，先挑一个低风险的事情，比如改一小段文档、补一个局部测试、修一个很小的 bug。等项目继续推进，再慢慢补充 AGENTS.md 和 skill。它们不需要一次写到完美，更好的方式是随着项目开发把**真实踩过的坑、反复出现的偏差、已经验证过的工作流**一点点沉淀进去。每次更新最好对应一个具体问题，而不是凭空加原则。比如 Agent 曾经误改过某个代码，就把这个代码的处理方式写清楚，某次测试命令经常被漏跑，就把它放进任务验收要求。

后续可以按任务类型逐步使用其他 skill。你可以直接让**Codex 基于这套模板 skill 结合自己项目的实际需求**生成专属于这个项目的 skill。普通开发任务，用 [project-prompt-creator](https://github.com/Philip-Cao-9527/code-note-helper/blob/main/vibe-coding-template/skills/project-prompt-creator/SKILL.md) 把任务需求写清楚。方向不明确的任务，先用 [plan-mode-planner](https://github.com/Philip-Cao-9527/code-note-helper/blob/main/vibe-coding-template/skills/plan-mode-planner/SKILL.md) 做关键决策。代码改完后，用 [code-reviewer](https://github.com/Philip-Cao-9527/code-note-helper/blob/main/vibe-coding-template/skills/code-reviewer/SKILL.md) 检查代码里的风险隐患。遇到官方文档、前沿技术等容易被模型幻觉影响的内容，就先用 [web-search](https://github.com/Philip-Cao-9527/code-note-helper/blob/main/vibe-coding-template/skills/web-search/SKILL.md) 建立依据。自己也没完全理解的技术点，再用 [knowledge-explainer](https://github.com/Philip-Cao-9527/code-note-helper/blob/main/vibe-coding-template/skills/knowledge-explainer/SKILL.md) 先补齐技术原理。

这样迁移的好处是模板不会变成一堆一次性文件，而是会逐渐变成项目自己的协作资产。对于开源项目来说，这些内容也能帮助后来参与的人更快理解这个项目应该怎么改、怎么测试、怎么 code review。
