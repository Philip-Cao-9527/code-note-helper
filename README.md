...
- 辛辛苦苦刷了两百多道题，过了一个月回头看却发现忘得一干二净，因为当时根本没有沉淀下可以用来复习的笔记。
...
**和 AI 长时间对话的时候：**
- 一个题单三十多道题都在同一个对话里问，聊到后面对话窗口已经几十屏了，想找之前某道题的回答只能不停地滚轮翻页。
...
- **直连 API 流式输出**：配置好 API 后，可在题目页面内直接生成笔记，支持实时流式响应，所见即所得。
...
- **题型智能分流**：自动识别 Attention、Normalization、Training Loop 等题型，生成对应的检查清单和面试深挖考点。
...
> - **TorchCode 页面没有按钮？**：确认你已经进入了具体的 notebook 页面（代码编辑区），而不是停留在项目介绍页。
