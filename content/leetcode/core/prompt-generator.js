...
请针对以上用户疑问，使用与完整笔记模式一致的回应范式输出：
1. [结论] + [原因] + [具体步骤] + [边界/反例] + [常见误区]
2. [继续逐条回答，每条都要落到用户原文对应的问题点]
3. [若涉及实现问题，必须给出可运行代码 + 修改点清单 + 为什么这样改]
...
4. **【允许的修改】**
   ...
   - ✅ 修改极度难懂的变量名（需标注原变量名）
   - ✅ 调整代码格式和缩进（保持逻辑不变）
   - ✅ **语言翻译【强制要求】**：如果参考题解语言与【我的代码】语言不一致，必须翻译为【我的代码】语言；允许在目标语言中做“等价且更自然”的实现优化（如更惯用语法/标准库写法/更清晰结构），但不得改变算法目标与复杂度结论。
   - 若发生翻译，必须额外输出“【语言翻译说明】”，且包含：
   - 必须明确标注：原始语言 -> 目标语言，以及触发翻译原因（与【我的代码】语言保持一致）
   - 必须明确标注“保留项（与原逻辑一致）”和“优化项（目标语言更优写法）”，逐条解释差异与收益
   - 必须提供“翻译对照说明”：关键语法、标准库调用、数据结构写法如何等价映射，避免生硬直译
