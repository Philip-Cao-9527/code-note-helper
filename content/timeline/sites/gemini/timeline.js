...
(function () {
    ...
    const ADAPTER = {
        ...
        isConversationPage: (path) => {
            // 匹配 /app/<id>、/share/<id>、/u/<index>/gem/<gemId>/<conversationId>，以及新对话起始页 /app 和 /app/
            return /\/app\/[A-Za-z0-9_-]+/.test(path) ||
                /\/share\/[A-Za-z0-9]+/.test(path) ||
                /\/u\/\d+\/gem\/[A-Za-z0-9_-]+(?:\/[A-Za-z0-9_-]+)?/.test(path) ||
                path === '/app' ||
                path === '/app/';
        },
        userMessageSelector: 'user-query',
        aiMessageSelector: 'message-content',
        extractAIText: (el) => {
            const content = el.querySelector('.markdown-main-panel, .response-content, .markdown');
            return (content?.textContent || el.textContent || '').replace(/\s+/g, ' ').trim();
        },
        getConversationId: () => {
            const appMatch = location.pathname.match(/\/app\/([A-Za-z0-9_-]+)/);
            if (appMatch) return appMatch[1];

            const gemMatch = location.pathname.match(/\/u\/\d+\/gem\/([A-Za-z0-9_-]+)\/([A-Za-z0-9_-]+)/);
            if (gemMatch) return `gem-${gemMatch[1]}-${gemMatch[2]}`;

            return null;
        },
        timelinePosition: { top: '80px', right: '16px', bottom: '100px' },
        scrollContainerSelector: 'infinite-scroller.chat-history, .chat-history, .content-container',
        extractUserText: (el) => {
            const queryText = el.querySelector('.query-text');
            if (queryText) return queryText.textContent.replace(/\s+/g, ' ').trim();
            const clone = el.cloneNode(true);
            clone.querySelectorAll('.link-preview, .url-card, .error-message, a[href^="http"], button').forEach(n => n.remove());
    ...
    class TimelineController {
        ...
        setupEventListeners() {
            ...
            // Gemini 分享页面备份方案：分享页面的滚动容器与普通页面不同
            if (this.adapter.name === 'Gemini' && location.pathname.includes('/share/')) {
                window.addEventListener('scroll', this.handleScroll, { passive: true, capture: true });
                console.log('[AI Timeline] 已添加 Gemini 分享页全局滚动监听');
            }
        }
