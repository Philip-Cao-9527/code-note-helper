/**
 * AI 对话时间轴 内容脚本 - gpt.aimonkey.plus
 * 版本：1.0.80
...
(function () {
    ...
    const ADAPTER = {
        name: 'AiMonkey-GPT',
        matches: (url) => url.includes('gpt.aimonkey.plus'),
        isConversationPage: (path) => path.includes('/c/') || path === '/' || path === '',
        ...
        getConversationId: () => {
            ...
            if (cIndex !== -1 && segs[cIndex + 1]) return segs[cIndex + 1];
            return null;
        },
        timelinePosition: { top: '100px', right: '48px', bottom: '100px' },
